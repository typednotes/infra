...
def create (creds : Credentials) (project name value markerValue : String)
    (extraLabels : List (String × String) := []) : IO String := do
  let labels := (markerKey, markerValue) :: extraLabels
  discard <| Gcp.call creds "POST" host s!"/v1/projects/{project}/secrets"
    [("secretId", some name)]
    (payload := some (.object
      [ ("replication", .object [("automatic", .object [])])
      , ("labels", .object (labels.map fun (k, v) => (k, Value.string v))) ]))
