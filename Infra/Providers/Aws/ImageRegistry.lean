...
/-
  AWS image registry (ECR), no matching cross-cutting Abstraction. Defines its own
  `TargetState`/`RemoteState`/`Diffable` pair, following `Infra.Providers.Scaleway.Iam`'s
  worked pattern.
-/

namespace Infra.Providers.Aws.ImageRegistry
...
instance : Keyed RemoteState where
  key s := { provider := "aws", service := "image-registry", id := s.id }
...
end Infra.Providers.Aws.ImageRegistry
