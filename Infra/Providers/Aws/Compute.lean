...
/-
  AWS serverless compute (Lambda), backed entirely by `Infra.Abstractions.ServerlessCompute` —
  see `docs/architecture.md`'s Abstractions section. No live AWS client yet: `Backend` is an
  empty placeholder handle and every method returns an honest placeholder value.
-/

namespace Infra.Providers.Aws.Compute
...
end Infra.Providers.Aws.Compute
