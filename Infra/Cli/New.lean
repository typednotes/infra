...
private def infraRev : String := "v0.17.1"
...
/-!
  ...
  1. **Removing a `resource` line destroys the resource** on the next
     `apply`, from any machine: everything this fleet creates carries its
     marker, and a marked resource the file no longer names is an orphan.
     To stop managing something without deleting it, `forget` it — and keep
     the `forget` line for as long as the resource exists.
  2. **Only what carries this fleet's marker is touched.** Everything else in
     your accounts — including a resource that happens to have a name you
     declare — is left alone. There is no import step.
...
it never enters the repository, the plan output or a `dump`.
...
- **Rename the resources** in `Fleet.lean`. Some names are globally unique
  per cloud.
- **Nothing is stored locally.** What this fleet manages is marked on the
  resources themselves and read back on every run, so there is no state file
  to commit, lose or share — any machine reaches the same plan.

## Two things that surprise people

- Removing a `resource` line **deletes** the resource on the next `apply`,
  from any machine. Run `plan` first: it lists every such DELETE. To keep a
  resource but stop managing it, `forget` it and leave the `forget` line in.
- Only resources carrying this fleet's marker are changed or deleted. A
  resource you did not create with this fleet is left alone even if its name
  matches one in `Fleet.lean` — `plan` warns about it by name.
"
